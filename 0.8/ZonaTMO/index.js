...
Object.defineProperty(exports, "__esModule", { value: true });
exports.ZonaTMO = exports.ZonaTMOInfo = void 0;
...
class ZonaTMO extends types_1.Source {
    ...
    parseLibraryCards(html) {
        ...
        $("div.element").each((_i, el) => {
            ...
            if (!mangaId || seen.has(mangaId))
                return;
            const title = card.find("h4.text-truncate").attr("title")?.trim() ||
                card.find("h4.text-truncate").text().trim();
            const image = card.find("img.cover-bg-img").attr("src") ||
    ...
    async getMangaDetails(mangaId) {
        ...
        const title = titleEl.contents().filter((_i, el) => el.type === "text").text().trim() ||
            titleEl.text().trim() ||
            mangaId;
        ...
        $("h6 a.badge.badge-primary").each((_i, el) => {
            ...
            if (label)
                tags.push(createTag({ id: label.toLowerCase().replace(/\s+/g, "-"), label, type: 'blue' }));
        });
        // Tipo (manga/manhwa/...) desde el propio ID
        const type = mangaId.match(/^([a-z_]+)\//)?.[1];
        if (type)
            tags.push(createTag({ id: type, label: type.toUpperCase(), type: 'default' }));
        return createManga({
            id: mangaId,
            titles: [title],
            image: image,
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
        ...
        if (homepageSectionId === 'popular')
            url = `${WEB_URL}/biblioteca?order_item=likes_count&order_dir=desc&filter_by=title&page=${page}`;
        else if (homepageSectionId === 'latest_added')
            url = `${WEB_URL}/biblioteca?order_item=creation&order_dir=desc&filter_by=title&page=${page}`;
        else
